...
/*   Updated: 2017/04/25 14:26:02 by wwatkins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_julia(t_env *e, int x, int y)
{
	...
	{
		e->f.y = (e->f.x + e->f.x) * e->f.y + e->f.c_im;
	...
	if (e->key.o)
		...
		ft_imgpixelput(e, x, y, smooth_shading(&e->f, &iteration, 3));
	else
		iteration == e->f.max ? ft_imgpixelput(e, x, y, ft_getcolor(e, -1)) :
...
void	ft_mandelbrot(t_env *e, int x, int y)
{
	...
	e->f.x2 = 0;
	e->f.y2 = 0;
	e->f.c_re = (x - e->hwin_w) / (e->hwin_w * e->f.zoom) + e->f.offx;
	e->f.c_im = (y - e->hwin_h) / (e->hwin_h * e->f.zoom) + e->f.offy;
	iteration = 0;
	while (e->f.x2 + e->f.y2 < 4 && iteration < e->f.max)
	{
		e->f.y = (e->f.x + e->f.x) * e->f.y + e->f.c_im;
