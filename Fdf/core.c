...
/*   Updated: 2015/12/21 22:06:36 by wwatkins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_assigncoor(t_env e, t_point **pts, int **tab, int tabwidth)
{
	...
	while (tab[y] != NULL)
	{
		...
		while (x < tabwidth)
		{
			pts[y][x].x = (x * 1.2 - y * 1.5) * 20 + 600;
			pts[y][x].y = (y * 1.2 + x * 1.5) * 20 - (tab[y][x] * 4) + 200;
			if (x > 0)
				ft_drawline(e, ft_line(pts[y][x].x, pts[y][x].y,
				pts[y][x - 1].x, pts[y][x - 1].y), 0x41AAAA - tab[y][x] * 9);
			if (y > 0)
				ft_drawline(e, ft_line(pts[y][x].x, pts[y][x].y,
				pts[y - 1][x].x, pts[y - 1][x].y), 0x41AAAA - tab[y][x] * 9);
			x++;
		}
		y++;
	}
